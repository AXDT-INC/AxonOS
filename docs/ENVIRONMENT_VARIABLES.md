...
| `AXGT_HOST_PROC_DIR` | `/host/proc` | Where the launcher container finds the host procfs (compose bind-mounts `/proc` there read-only). Used by `/session-process-map` to translate NVML host PIDs into a session's own PIDs for the in-session `nvidia-smi` wrapper; without the mount the map carries names but no container PIDs. |
