...
| `WEBRTC_GATE_INTERNAL_URL` | Launcher | Internal-only agent API. Launcher-managed sessions use `http://axonos:8890`; legacy single-container mode uses loopback. |
| `AXGT_GATE_HEARTBEAT_URL` | Launcher | Central gate URL the heartbeat daemon posts to. Launcher-managed sessions use `http://axonos:8889`. |
...
| `WEBRTC_GATE_INTERNAL_URL` | `http://127.0.0.1:8890` | Agent-only gate URL. Launcher-managed sessions are forced to `http://axonos:8890`; the internal listener is not host-published. |
