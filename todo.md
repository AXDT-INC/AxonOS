...
- [x] **USDC + x402 payment rail + dynamic USD pricing + agent SSH (2026-06-16)** — Base Sepolia
  ...
  - [x] **Heartbeat for headless / SSH-only sessions (2026-06-16)** — browser-less sessions (human SSH-toggle + agent SSH) had NO heartbeat driver (only novnc ui.js sent them), so they were reaped after AXGT_HEARTBEAT_TIMEOUT_SECONDS (~120s) and barely billed. Confirmed live (session 142 ended at ~100s with ~57min paid left). The initial fix let `/api/session/heartbeat` accept the per-session `files_key` (X-AXGT-Session-Key header) via `validate_session_files_key` and ran `session_heartbeat_daemon.py` for headless containers. **Superseded 2026-07-28:** the daemon now heartbeats every tenant container, including desktops, so Detach/reload/tab close does not change runtime liveness or billing. Gate-side proven: files_key heartbeat authenticates, resets last_heartbeat, bills correctly; wrong/cross-wallet keys → 401. **Needs session-image rebuild** to ship daemon changes into containers.
...
- [x] **Storage/SSH/telemetry regression fixes on `frontend-revamp` (2026-07-09)** — verified the v0.2 investigation report, fixed the confirmed items (one claim was already fixed):
  ...
  - [x] **Server pauses (not ends) heartbeat-stale sessions**: `_pause_stale_sessions` (was `_pause_stale_zero_credit_sessions`) now pauses ALL heartbeat-stale active sessions when `AXGT_SESSION_PRESERVE_ON_CREDIT_EXHAUST` (default on) — container/storage preserved, billing stops (heartbeat-driven), resumable within `AXGT_SESSION_PAUSED_MAX_MINUTES` (2h default), then reaped. Without this the client-side fix was moot: any detached desktop was `ended` ~2min after tab close by the heartbeat reaper. Trade-off: an abandoned session holds its GPU for the paused TTL (operator-tunable).
  - [x] **Wallet switch / sign-out releases non-viewer sessions**: `teardownSessionForWalletChange()` no longer early-returns when no live viewer — owned SSH/detached/paused state (`axonosOwnedSession`/`axonosDetachedSession`/`axonosSessionDetached`/`axonosPausedResume`) also triggers `UI.disconnect()` (releases active AND paused server-side) + SSH card hide.
  ...
  - [x] Verified: 220 gate tests pass; inline JS + ui.js syntax-checked (node container); end-to-end vs throwaway Postgres: legacy-schema migration, ssh field round-trip, stale→paused (container kept), release ends paused, preserve-off still ends; file_agent `/stats` HTTP round-trip (403 without key, real cgroup values with key). **Needs session-image rebuild** (file_agent /stats) + gate rebuild to deploy.
...
- [x] **x402 SSH-session audit of the last two commits (2026-07-09)** — verified `/api/x402/session` against cbd4c13 + 535cc54; found and fixed two gaps:
  ...
  - [x] **Paused-SSH resume was broken for both rails**: `_resume_paused_session` returned no ssh fields (an agent re-claiming a paused session lost its endpoint) and the browser resume path (`axonosTryResumeDesktopAfterCredit`) blind-called `tryConnectAfterClaim()` → desktop connect against a headless container. Resume now renews the cap (extend-only), returns `ssh_*` + `hard_cap_remaining_seconds`, and the browser routes SSH resumes to the connect-card restore.
