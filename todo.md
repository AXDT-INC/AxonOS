...
## Code-side inconsistencies surfaced by the 2026-09-06 docs audit (docs already aligned; code untouched)

- [ ] `AXGT_GPU_PROFILES_ENABLED` default is **false** in `axonos_gate/axgt_verifier.py:728` but **true** in `session_manager.py:97`, `gate_server.py:1603`, `websockify_gate.py:1308` → `/api/config.gpu_weighted_billing_enabled` reports false unless set explicitly.
- [ ] `AXGT_ENABLE_USDC_DEPOSITS`: positive allowlist in `x402_verifier.py:41-46` vs falsy-list in `axgt_verifier.py:713`; non-canonical values report enabled but reject deposits.
- [ ] `AXONOS_HIDE_BETA_BADGE` truthiness differs between `gate_server.py:1610` and `websockify_gate.py:1317`. Consider renaming the env var / `hide_beta_badge` config key for the launch release (docs now call it the "release-stage chip").
- [ ] `GATE_USE_GEVENT` (`gate_server.py:3326`) does not accept `on`, unlike the repo-wide boolean convention.
- [ ] `AXGT_EXPECTED_CONTRACT_ADDRESS` is read by no Python code (still in `env.example:506`); `AXONOS_TEST_CREDIT_MAX_BALANCE_MINUTES` is a dead knob for the wallet button (`axgt_verifier.py:1023` uses `additive=True`).
- [ ] `session_launcher_service.py:1877-2040` storage sweep bills every retained volume regardless of session state but labels ledger rows "Offline storage charge"; `_get_volume_size_kb` (`:1848-1861`) returns 0 on failure (e.g. `alpine` not pullable) and silently bills nothing.
- [ ] `x402_verifier.py:531` hard-codes `https://app.axonos.io` as the AgentLink resource fallback; derive from `AXGT_PUBLIC_BASE_URL` / `AXONOS_PUBLIC_ORIGIN`.
- [ ] `webrtc/capture.py:152-156` leaves `WEBRTC_CAPTURE_FPS` unclamped.
- [ ] `websockify_gate.py:1578,2099` section comments still say "Session / Queue"; no queue routes exist. `gate_server.py:1596-1602` duplicates `_multi_session_enabled()` instead of importing it.
- [ ] `tools/x402-agent-test/agent.mjs:7-8` comment claims a v2 body by default (gate defaults to v1 body + v2 header).
- [x] Image tag `axonos:public-beta` → `axonos:latest` (2026-09-09) in `docker-compose.yml:32,132,141` and `env.example:14,438,468`: rename for the launch release (deploy-affecting; docs refer to it as "the image tag set in docker-compose.yml").
- [ ] Build tooling (`build/`): Makefile targets run scripts without the `build/` prefix while scripts require repo-root cwd (only `make cross` works); `Makefile:47` .deb glob never matches and uses the stale `axonos-launcher_*` name (`build_deb.py:259` emits `AxonOS-Launcher-0.1.0-Linux-amd64.deb`); `build_windows.py:52` `import pyyaml` → `yaml`; `build_simple_cross.py:232` packages `dist/axonos` for both targets and claims ELF runs on macOS; `build_cross_platform.py` cannot succeed (platform checks inside Linux containers); `MyDocument.icns` referenced but absent; PyInstaller specs use `console=False` although the launcher is CLI-by-default; `build_all.py:179-180`/`build_deb.py:84,190` still point at `GizmoQuest/AxonOS`; version `0.1.0` duplicated in 11 files.
- [ ] Launcher Dockerfile generation is likely stale: `launcher_core.py:387` / `main.py:1439` search for marker `'Install pip for system Python'` which no longer exists in `Dockerfile`, so a custom generate copies the whole Dockerfile and appends duplicate sections; Ollama model substitution (`launcher_core.py:494`) expects a single-line `ollama pull ... RUN ollama serve`. `cli.py:174` epilog shows `axonos build --gpu` (no such flag). `.deb` desktop entry `Exec=axonos` (`build_deb.py:99`) opens the CLI, not the GUI.
- [ ] AxonAI MCP sidecar: `mcp_client.py:190-193` never opens MCP sessions (stub); `main.py:675-702` closes the asyncio loop right after start so the 2 s context loop is never driven; `mcp_client.py:739-802` `execute_os_command`/`get_file_context` are unrestricted (unreachable from UI); `mcp_filesystem_server.py:143` prefix check matches `/homeX`, `:201`/`:241` `created`/`type` computed after the operation; dead handlers `main.py:2553,2601,2795,2904`; `mcp_client.py:84` forces DEBUG logging and `:93` probes `/home/avi/...`; `wmctrl` not installed in the image; `opencode.json` has no `mcp` block; model/agent-mode settings not persisted.
- [ ] `novnc-theme/app/webrtc/axonos-webrtc-input-validation.js` is not copied into the image by the Dockerfile; `@keyframes rocket-pulse` referenced at `axonos-theme.css:3608` is undefined; `axonos_plugins/example_bioinformatics.yaml:13` uses `gnome-terminal`, which the image does not ship.

## Multiple concurrent sessions per wallet (2026-09-07) — IMPLEMENTED incl. follow-ups, pending deploy

- [x] Server: `try_claim_session(new_session=True)` allocates an additional session beside owned ones; non-resume `expected_session_id` is an exact reattach (mismatch never spawns); legacy claims keep reattach-newest. Heartbeat/release/files/terminal/WebRTC resolve the exact session (`session_id`, `session_id_for_files_key`, `compute_session_id`); unscoped release tears down every session; `session_status` adds `owned_sessions`, `owned_session_ids`, `free_gpu_count`, `total_gpu_count`, per-row ssh fields on own `active_sessions` rows; `billing_context_for_wallet` sums GPU rates.
- [x] Client: wizard greys out only tiers exceeding free GPUs (credit-grace resume still pins), wizard launch sends `new_session`, every claim/heartbeat/files/terminal call names `axonosCurrentSessionId()`, dashboard "Open"/SSH row/End act on their own card's session, status reducer binds to the current row (a paused sibling no longer hijacks reconnects).
- [x] Dashboard card title/icon: `requested_template` is now stored on the session row (additive migration) and returned in `owned_sessions`/own `active_sessions` rows; cards label themselves from it (rows predating the column fall back to the last wizard pick).
- [x] Storage: while any sibling session runs (`axonosGpuCapacity.ownedSessions > 0`, not a credit-grace resume) the wizard slider is locked at the provisioned floor with a "shared volume, resize after every session ends" hint, so a launch can no longer fail at spawn with "Cannot expand volume while a container is using it". Siblings still share the one loop-ext4 home volume by design.
- [x] Per-row grace resume: `owned_sessions` lists every paused sibling with `credit_grace_remaining_seconds` + `resume_minutes_required`; dashboard renders one card per paused row and its Top up / Reconnect binds the resume marker to THAT row (`axonosSetPausedResumeFromRow`) and passes its id; `resume_only` claims look the expected id up exactly instead of taking the newest grace row. `credit_grace_*` scalars still describe the newest paused row for older clients.
- [x] SSH ports come from a pool: `_allocate_ssh_port` (under the scheduler lock) picks the lowest port in 42000-42049 not held by a live SSH session (legacy NULL-port rows keep occupying their id-derived port), stores it in the new `ssh_port` column, hands it to the launcher (`ssh_port` in the /launch payload → `-p <port>:22/tcp`), and denies the claim with `ssh_ports_exhausted` when the pool is full. `_ssh_connection_fields` prefers the stored port and derives only for legacy rows.
- [x] Shared-home concurrency guards (session image, needs rebuild): `/usr/local/bin/firefox` wrapper falls back to a per-session profile under `/var/lib/axonos-desktop` when the shared profile's `.parentlock` is fcntl-held by a sibling session; the IPFS supervisord program probes `repo.lock` and idles instead of crash-looping when another session serves the repo; xfce runs with a container-local `ICEAUTHORITY`. `.Xauthority` stays on the home volume (xauth entries are keyed per container hostname, so siblings do not clobber each other).
- [x] Session cards: inline-editable title (pencil / double-click, Enter saves, Escape cancels), specs line "Region / GPU tier · N GPUs / Environment", stats "Up 29m · 1 cr/min", expandable 📝 notes footer; title/notes persist on the session row (`title`, `notes` columns) via `POST /api/session/annotate` on both gates (owner-only, live rows, 80/2000 chars, control chars stripped); region from `AXONOS_HOST_REGION` → `session_status.host_region`. Guests can annotate their demo session too (same auth token path); paused cards show but do not edit.
- [x] Wizard summary rail shows the combined burn rate ("Total with N running sessions") whenever the wallet already runs sessions, so launching a 4x beside a 1x never surprises anyone about remaining time.
- [ ] 2026-09-07 live test (4 rapid launches on the old image): the 4th session's first two WebRTC attempts negotiated onto a relay→relay ICE pair (agent relay → browser relay on the local coturn) within ~1 s, before the direct srflx/prflx checks completed, and decoded no video in 15 s; the later workspace retry landed on the direct pair and worked. Session 448 also connected relay→relay and worked, so the relay path is flaky rather than broken. Added diagnostics: the client's no-video error now names the path and inbound packet/frame counts; the agent logs connection state and video-sender packets at +3 s/+10 s. Next: capture one failure with these, then decide between preferring direct pairs (delay relay candidates / ICE restart on relay+no-frames) and fixing coturn relay→relay media.
- [x] Loader wording: an additional-session wizard launch no longer shows "Checking running session access / Claiming running GPU container / Restarting desktop video" (those are reattach steps); `axonosNewSessionLaunchPending` keeps the fresh-launch labels until the loader hides.
- [x] False "Session end not confirmed" after ending several sessions in a row (2026-09-07 live test): the dashboard End used the generic 20 s fetch deadline while the release endpoint stops the container synchronously and sibling releases queue behind the scheduler lock. Dashboard End now uses the release budget, reconciles a timed-out/unreachable reply against status before blaming the server (per-session: confirmed when THAT id is gone, whatever siblings do), and any stale banner clears on the next dashboard load that no longer lists the session.

## WebRTC video bitrate is fixed and loss-insensitive (from 2026-09-09 far-user demo drop investigation)

Context: a demo from a far region over a screen-share call saw the viewer reconnect repeatedly on a day the local uplink was already flaky. Diagnosis with `chrome://webrtc-internals` on a good day: direct prflx↔srflx pair (no relay), RTT steady ~280 ms, 642/642 consent checks answered, yet `packetsLost` climbed in bursts with `nackCount`/`pliCount` rising continuously and a 0–40 fps sawtooth while the decoder waited for keyframes. The viewer survived; the screen-share call dropped first. Root cause of the bad day was path loss, but the stream shape amplifies it.
